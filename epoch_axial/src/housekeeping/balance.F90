...
CONTAINS
  ...
  SUBROUTINE redistribute_fields(new_domain)
    ...
    COMPLEX(num), DIMENSION(:,:,:), ALLOCATABLE :: temp_zsum
    COMPLEX(r4), DIMENSION(:,:,:), ALLOCATABLE :: r4temp_zsum
    REAL(num), DIMENSION(:,:,:), ALLOCATABLE :: temp_sum
    REAL(r4), DIMENSION(:,:,:), ALLOCATABLE :: r4temp_sum
    REAL(num), DIMENSION(:,:), ALLOCATABLE :: temp, temp2
    COMPLEX(num), DIMENSION(:,:,:), ALLOCATABLE :: temp_cmplx, temp2_cmplx
    REAL(num), DIMENSION(:), ALLOCATABLE :: temp_slice
    COMPLEX(num), DIMENSION(:,:), ALLOCATABLE :: temp_mode_slice
    ...
    INTEGER :: i, im, ispecies, io, id, nspec_local, mask
    ...
    ALLOCATE(temp_cmplx(1-ng:nx_new+ng, 1-ng:ny_new+ng, 0:n_mode-1))
    ...
    IF (overriding) THEN
      ...
      ALLOCATE(temp2_cmplx(1-ng:nx+ng, 1-ng:ny+ng, 0:n_mode-1))
      ...
      temp2_cmplx = jxm
      CALL remap_field_modes(temp2_cmplx, temp_cmplx)
      DEALLOCATE(jxm)
      ALLOCATE(jxm(1-jng:nx_new+jng, 1-jng:ny_new+jng, 0:n_mode-1))
      jxm(0:nx_new+1, 0:ny_new+1, :) = temp_cmplx(0:nx_new+1, 0:ny_new+1, :)

      temp2_cmplx = jrm
      CALL remap_field_modes(temp2_cmplx, temp_cmplx)
      DEALLOCATE(jrm)
      ALLOCATE(jrm(1-jng:nx_new+jng, 1-jng:ny_new+jng, 0:n_mode-1))
      jrm(0:nx_new+1, 0:ny_new+1, :) = temp_cmplx(0:nx_new+1, 0:ny_new+1, :)
      
      temp2_cmplx = jtm
      CALL remap_field_modes(temp2_cmplx, temp_cmplx)
      DEALLOCATE(jtm)
      ALLOCATE(jtm(1-jng:nx_new+jng, 1-jng:ny_new+jng, 0:n_mode-1))
      jtm(0:nx_new+1, 0:ny_new+1, :) = temp_cmplx(0:nx_new+1, 0:ny_new+1, :)

      temp2_cmplx = jxm_old
      CALL remap_field_modes(temp2_cmplx, temp_cmplx)
      DEALLOCATE(jxm_old)
      ALLOCATE(jxm_old(1-jng:nx_new+jng, 1-jng:ny_new+jng, 0:n_mode-1))
      jxm_old(0:nx_new+1, 0:ny_new+1, :) = temp_cmplx(0:nx_new+1, 0:ny_new+1, :)

      temp2_cmplx = jrm_old
      CALL remap_field_modes(temp2_cmplx, temp_cmplx)
      DEALLOCATE(jrm_old)
      ALLOCATE(jrm_old(1-jng:nx_new+jng, 1-jng:ny_new+jng, 0:n_mode-1))
      jrm_old(0:nx_new+1, 0:ny_new+1, :) = temp_cmplx(0:nx_new+1, 0:ny_new+1, :)
      
      temp2_cmplx = jtm_old
      CALL remap_field_modes(temp2_cmplx, temp_cmplx)
      DEALLOCATE(jtm_old)
      ALLOCATE(jtm_old(1-jng:nx_new+jng, 1-jng:ny_new+jng, 0:n_mode-1))
      jtm_old(0:nx_new+1, 0:ny_new+1, :) = temp_cmplx(0:nx_new+1, 0:ny_new+1, :)

      DEALLOCATE(temp2, temp2_cmplx)
    ELSE
      DEALLOCATE(jx)
      DEALLOCATE(jy)
      DEALLOCATE(jz)
    ...
    CALL remap_field_modes(exm, temp_cmplx)
    DEALLOCATE(exm)
    ALLOCATE(exm(1-ng:nx_new+ng, 1-ng:ny_new+ng, 0:n_mode-1))
    exm = temp_cmplx

    CALL remap_field_modes(erm, temp_cmplx)
    DEALLOCATE(erm)
    ALLOCATE(erm(1-ng:nx_new+ng, 1-ng:ny_new+ng, 0:n_mode-1))
    erm = temp_cmplx

    CALL remap_field_modes(etm, temp_cmplx)
    DEALLOCATE(etm)
    ALLOCATE(etm(1-ng:nx_new+ng, 1-ng:ny_new+ng, 0:n_mode-1))
    etm = temp_cmplx

    CALL remap_field_modes(bxm, temp_cmplx)
    DEALLOCATE(bxm)
    ALLOCATE(bxm(1-ng:nx_new+ng, 1-ng:ny_new+ng, 0:n_mode-1))
    bxm = temp_cmplx

    CALL remap_field_modes(brm, temp_cmplx)
    DEALLOCATE(brm)
    ALLOCATE(brm(1-ng:nx_new+ng, 1-ng:ny_new+ng, 0:n_mode-1))
    brm = temp_cmplx

    CALL remap_field_modes(btm, temp_cmplx)
    DEALLOCATE(btm)
    ALLOCATE(btm(1-ng:nx_new+ng, 1-ng:ny_new+ng, 0:n_mode-1))
    btm = temp_cmplx

    CALL remap_field_modes(bxm_old, temp_cmplx)
    DEALLOCATE(bxm_old)
    ALLOCATE(bxm_old(1-ng:nx_new+ng, 1-ng:ny_new+ng, 0:n_mode-1))
    bxm_old = temp_cmplx

    CALL remap_field_modes(brm_old, temp_cmplx)
    DEALLOCATE(brm_old)
    ALLOCATE(brm_old(1-ng:nx_new+ng, 1-ng:ny_new+ng, 0:n_mode-1))
    brm_old = temp_cmplx

    CALL remap_field_modes(btm_old, temp_cmplx)
    DEALLOCATE(btm_old)
    ALLOCATE(btm_old(1-ng:nx_new+ng, 1-ng:ny_new+ng, 0:n_mode-1))
    btm_old = temp_cmplx
    ...
    DO id = 1, num_vars_to_dump
      ...
      IF (io_block_list(io)%averaged_data(id)%dump_single) THEN
        ...
        IF (.NOT. ASSOCIATED(io_block_list(io)%averaged_data(id)%r4zarray)) CYCLE
        SELECT CASE (id)
        CASE (c_dump_exm, c_dump_erm, c_dump_etm)
          ALLOCATE(r4temp_zsum(1-ng:nx_new+ng, 1-ng:ny_new+ng, 0:n_mode-1))
          CALL remap_field_modes_r4(&
              io_block_list(io)%averaged_data(id)%r4zarray(:,:,:), &
              r4temp_zsum(:,:,:))
        CASE DEFAULT
        ...
        SELECT CASE (id)
        CASE (c_dump_exm, c_dump_erm, c_dump_etm)
          DEALLOCATE(io_block_list(io)%averaged_data(id)%r4zarray)
          ALLOCATE(io_block_list(io)%averaged_data(id)&
              %r4zarray(1-ng:nx_new+ng, 1-ng:ny_new+ng, 0:n_mode-1))
          io_block_list(io)%averaged_data(id)%r4zarray = r4temp_zsum
          DEALLOCATE(r4temp_zsum)
        CASE DEFAULT
      ...
      ELSE
        ...
        IF (.NOT. ASSOCIATED(io_block_list(io)%averaged_data(id)%zarray)) CYCLE
        SELECT CASE (id)
        CASE (c_dump_exm, c_dump_erm, c_dump_etm)
          ALLOCATE(temp_zsum(1-ng:nx_new+ng, 1-ng:ny_new+ng, 0:n_mode-1))
          CALL remap_field_modes(&
              io_block_list(io)%averaged_data(id)%zarray(:,:,:), &
              temp_zsum(:,:,:))
        CASE DEFAULT
        ...
        SELECT CASE (id)
        CASE (c_dump_exm, c_dump_erm, c_dump_etm)
          DEALLOCATE(io_block_list(io)%averaged_data(id)%zarray)
          ALLOCATE(io_block_list(io)%averaged_data(id)&
              %zarray(1-ng:nx_new+ng, 1-ng:ny_new+ng, 0:n_mode-1))
          io_block_list(io)%averaged_data(id)%zarray = temp_zsum
          DEALLOCATE(temp_zsum)
        CASE DEFAULT
    ...
    ALLOCATE(temp_mode_slice(1-ng:ny_new+ng, 0:n_mode-1))
    ...
    CALL remap_field_mode_slice(c_dir_x, exm_x_min, temp_mode_slice)
    DEALLOCATE(exm_x_min)
    ALLOCATE(exm_x_min(1-ng:ny_new+ng, 0:n_mode-1))
    exm_x_min = temp_mode_slice

    CALL remap_field_mode_slice(c_dir_x, erm_x_min, temp_mode_slice)
    DEALLOCATE(erm_x_min)
    ALLOCATE(erm_x_min(1-ng:ny_new+ng, 0:n_mode-1))
    erm_x_min = temp_mode_slice

    CALL remap_field_mode_slice(c_dir_x, etm_x_min, temp_mode_slice)
    DEALLOCATE(etm_x_min)
    ALLOCATE(etm_x_min(1-ng:ny_new+ng, 0:n_mode-1))
    etm_x_min = temp_mode_slice

    CALL remap_field_mode_slice(c_dir_x, bxm_x_min, temp_mode_slice)
    DEALLOCATE(bxm_x_min)
    ALLOCATE(bxm_x_min(1-ng:ny_new+ng, 0:n_mode-1))
    bxm_x_min = temp_mode_slice

    CALL remap_field_mode_slice(c_dir_x, brm_x_min, temp_mode_slice)
    DEALLOCATE(brm_x_min)
    ALLOCATE(brm_x_min(1-ng:ny_new+ng, 0:n_mode-1))
    brm_x_min = temp_mode_slice

    CALL remap_field_mode_slice(c_dir_x, btm_x_min, temp_mode_slice)
    DEALLOCATE(btm_x_min)
    ALLOCATE(btm_x_min(1-ng:ny_new+ng, 0:n_mode-1))
    btm_x_min = temp_mode_slice

    max_boundary = .TRUE.

    CALL remap_field_mode_slice(c_dir_x, exm_x_max, temp_mode_slice)
    DEALLOCATE(exm_x_max)
    ALLOCATE(exm_x_max(1-ng:ny_new+ng,0:n_mode-1))
    exm_x_max = temp_mode_slice

    CALL remap_field_mode_slice(c_dir_x, erm_x_max, temp_mode_slice)
    DEALLOCATE(erm_x_max)
    ALLOCATE(erm_x_max(1-ng:ny_new+ng,0:n_mode-1))
    erm_x_max = temp_mode_slice

    CALL remap_field_mode_slice(c_dir_x, etm_x_max, temp_mode_slice)
    DEALLOCATE(etm_x_max)
    ALLOCATE(etm_x_max(1-ng:ny_new+ng,0:n_mode-1))
    etm_x_max = temp_mode_slice

    CALL remap_field_mode_slice(c_dir_x, bxm_x_max, temp_mode_slice)
    DEALLOCATE(bxm_x_max)
    ALLOCATE(bxm_x_max(1-ng:ny_new+ng,0:n_mode-1))
    bxm_x_max = temp_mode_slice

    CALL remap_field_mode_slice(c_dir_x, brm_x_max, temp_mode_slice)
    DEALLOCATE(brm_x_max)
    ALLOCATE(brm_x_max(1-ng:ny_new+ng,0:n_mode-1))
    brm_x_max = temp_mode_slice

    CALL remap_field_mode_slice(c_dir_x, btm_x_max, temp_mode_slice)
    DEALLOCATE(btm_x_max)
    ALLOCATE(btm_x_max(1-ng:ny_new+ng,0:n_mode-1))
    btm_x_max = temp_mode_slice

    DEALLOCATE(temp_slice)
    DEALLOCATE(temp_mode_slice)
  ...
  SUBROUTINE remap_field_mode_slice(direction, field_in, field_out)

    ! This is a wrapper for the field redistribution routine
    INTEGER, INTENT(IN) :: direction
    COMPLEX(num), DIMENSION(:,0:), INTENT(IN) :: field_in
    COMPLEX(num), DIMENSION(:,0:), INTENT(OUT) :: field_out
    INTEGER :: i, im, n
    INTEGER, DIMENSION(c_ndims-1) :: n_new, cdim

    DO im = 0, n_mode-1
      
      n_new = SHAPE(field_out(:,im)) - 2 * ng
      ...
      IF (direction == c_dir_x) THEN
        ...
        IF (max_boundary) THEN
          ...
          new_slice_coord = SIZE(new_cell_x_min) - 1
        END IF
        CALL redistribute_field_1d_cmplx(field_in(:,im), field_out(:,im), &
            cdim, cell_y_min, cell_y_max, new_cell_y_min, new_cell_y_max)
      ELSE
        slice_dir = 1
        IF (max_boundary) THEN
          old_slice_coord = SIZE(cell_y_min) - 1
        ...
        END IF
        CALL redistribute_field_1d_cmplx(field_in(:,im), field_out(:,im), &
            cdim, cell_x_min, cell_x_max, new_cell_x_min, new_cell_x_max)
      END IF

      CALL do_field_mpi_with_lengths_slice_cmplx(field_out(:,im), direction, &
          ng, n_new(1))
    END DO

  END SUBROUTINE remap_field_mode_slice
  ...
  SUBROUTINE remap_field_modes(field_in, field_out)

    ! Loop over all complex field modes in field_in, and write values to the 
    ! corresponding indices in field_out
    ...
    DO im = 0,n_mode-1
      CALL redistribute_field_2d_cmplx(field_in(:,:,im), field_out(:,:,im), &
          cdim, cell_x_min, cell_x_max, new_cell_x_min, new_cell_x_max, &
          cell_y_min, cell_y_max, new_cell_y_min, new_cell_y_max)

      CALL do_field_mpi_with_lengths_cmplx(field_out(:,:,im), ng, n_new(1), &
           n_new(2))
    END DO

  END SUBROUTINE remap_field_modes

  SUBROUTINE remap_field_modes_r4(field_in, field_out)

    ! Loop over all complex field modes in field_in, and write values to the 
    ! corresponding indices in field_out

    ! This is a wrapper for the field redistribution routine
    COMPLEX(r4), DIMENSION(:,:,0:), INTENT(IN) :: field_in
    COMPLEX(r4), DIMENSION(:,:,0:), INTENT(OUT) :: field_out
    INTEGER, DIMENSION(c_ndims) :: n_new, cdim
    INTEGER :: i, im
    ...
    DO im = 0,n_mode-1
      CALL redistribute_field_2d_cmplx_r4(field_in(:,:,im), field_out(:,:,im), &
          cdim, cell_x_min, cell_x_max, new_cell_x_min, new_cell_x_max, &
          cell_y_min, cell_y_max, new_cell_y_min, new_cell_y_max)

      CALL do_field_mpi_with_lengths_cmplx_r4(field_out(:,:,im), ng, n_new(1), &
           n_new(2))
    END DO

  END SUBROUTINE remap_field_modes_r4
  ...
  SUBROUTINE redistribute_field_1d_cmplx(field_in, field_out, cdim, &
      old_cell_min1, old_cell_max1, new_cell_min1, new_cell_max1)

    ! This subroutine redistributes the fields over the new processor layout
    INTEGER, PARAMETER :: nd = 1
    COMPLEX(num), DIMENSION(1-ng:), INTENT(IN) :: field_in
    COMPLEX(num), DIMENSION(1-ng:), INTENT(OUT) :: field_out
    INTEGER, DIMENSION(nd), INTENT(IN) :: cdim
    INTEGER, DIMENSION(:), INTENT(IN) :: old_cell_min1, old_cell_max1
    INTEGER, DIMENSION(:), INTENT(IN) :: new_cell_min1, new_cell_max1
    INTEGER :: irank, basetype, n, ng0, ng1
  ...
  END SUBROUTINE redistribute_field_1d_cmplx
  ...
  SUBROUTINE redistribute_field_2d_cmplx(field_in, field_out, cdim, &
      old_cell_min1, old_cell_max1, new_cell_min1, new_cell_max1, &
      old_cell_min2, old_cell_max2, new_cell_min2, new_cell_max2)

    ! This subroutine redistributes the fields over the new processor layout
    INTEGER, PARAMETER :: nd = 2
    COMPLEX(num), DIMENSION(1-ng:,1-ng:), INTENT(IN) :: field_in
    COMPLEX(num), DIMENSION(1-ng:,1-ng:), INTENT(OUT) :: field_out
    INTEGER, DIMENSION(nd), INTENT(IN) :: cdim
    INTEGER, DIMENSION(:), INTENT(IN) :: old_cell_min1, old_cell_max1
    INTEGER, DIMENSION(:), INTENT(IN) :: new_cell_min1, new_cell_max1
    INTEGER, DIMENSION(:), INTENT(IN) :: old_cell_min2, old_cell_max2
  ...
  END SUBROUTINE redistribute_field_2d_cmplx

  SUBROUTINE redistribute_field_2d_cmplx_r4(field_in, field_out, cdim, &
      old_cell_min1, old_cell_max1, new_cell_min1, new_cell_max1, &
      old_cell_min2, old_cell_max2, new_cell_min2, new_cell_max2)

    ! This subroutine redistributes the fields over the new processor layout
    INTEGER, PARAMETER :: nd = 2
    COMPLEX(r4), DIMENSION(1-ng:,1-ng:), INTENT(IN) :: field_in
    COMPLEX(r4), DIMENSION(1-ng:,1-ng:), INTENT(OUT) :: field_out
    INTEGER, DIMENSION(nd), INTENT(IN) :: cdim
    INTEGER, DIMENSION(:), INTENT(IN) :: old_cell_min1, old_cell_max1
    INTEGER, DIMENSION(:), INTENT(IN) :: new_cell_min1, new_cell_max1
    INTEGER, DIMENSION(:), INTENT(IN) :: old_cell_min2, old_cell_max2
  ...
  END SUBROUTINE redistribute_field_2d_cmplx_r4
...
#ifdef PARTICLE_SHAPE_TOPHAT
        cell = FLOOR((part_r - y_grid_min) / dy) + 1 + ng
#else
        cell = FLOOR((part_r - y_grid_min) / dy + 1.5_num) + ng
#endif
        load(cell) = load(cell) + 1
  ...
  SUBROUTINE part_load_func(load_x, load_y)
    ...
    DO ispecies = 1, n_species
      ...
      DO WHILE(ASSOCIATED(current))
        ...
        cell_y = FLOOR((part_r - y_grid_min) / dy) + 1
#else
        cell_x = FLOOR((current%part_pos(1) - x_grid_min) / dx + 1.5_num)
        cell_y = FLOOR((part_r - y_grid_min) / dy + 1.5_num)
#endif
  ...
  FUNCTION get_particle_processor(part)
    ...
    DO iproc = 0, nprocy - 1
      ...
      END IF
      part_r = SQRT(part%part_pos(2)**2 + part%part_pos(3)**2)
      IF (part_r >= minpos .AND. part_r < maxpos) THEN

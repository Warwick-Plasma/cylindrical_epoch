...
CONTAINS
  ...
  SUBROUTINE do_send1dcmplx(iproc)

    INTEGER, INTENT(IN) :: iproc
    INTEGER :: ierr

    IF (sendtypes(iproc) /= 0) THEN
      CALL MPI_SEND(field_in1dcmplx, 1, sendtypes(iproc), iproc, tag, comm, ierr)
    END IF

  END SUBROUTINE do_send1dcmplx



  SUBROUTINE do_recv1dcmplx(iproc)

    INTEGER, INTENT(IN) :: iproc
    INTEGER :: ierr

    IF (recvtypes(iproc) /= 0) THEN
      CALL MPI_RECV(field_out1dcmplx, 1, recvtypes(iproc), iproc, tag, comm, &
          MPI_STATUS_IGNORE, ierr)
    END IF

  END SUBROUTINE do_recv1dcmplx



  SUBROUTINE do_send2dcmplx(iproc)

    INTEGER, INTENT(IN) :: iproc
    INTEGER :: ierr

    IF (sendtypes(iproc) /= 0) THEN
      CALL MPI_SEND(field_in2dcmplx, 1, sendtypes(iproc), iproc, tag, comm, ierr)
    END IF
  ...
  SUBROUTINE do_recv2dcmplx(iproc)
    ...
    IF (recvtypes(iproc) /= 0) THEN
      CALL MPI_RECV(field_out2dcmplx, 1, recvtypes(iproc), iproc, tag, comm, &
          MPI_STATUS_IGNORE, ierr)
    END IF
  ...
  SUBROUTINE do_send3dcmplx(iproc)
    ...
    IF (sendtypes(iproc) /= 0) THEN
      CALL MPI_SEND(field_in3dcmplx, 1, sendtypes(iproc), iproc, tag, comm, ierr)
    END IF

  END SUBROUTINE do_send3dcmplx



  SUBROUTINE do_recv3dcmplx(iproc)

    INTEGER, INTENT(IN) :: iproc
    INTEGER :: ierr

    IF (recvtypes(iproc) /= 0) THEN
      CALL MPI_RECV(field_out3dcmplx, 1, recvtypes(iproc), iproc, tag, comm, &
          MPI_STATUS_IGNORE, ierr)
    END IF

  END SUBROUTINE do_recv3dcmplx
  ...
  SUBROUTINE redblack1d_cmplx(field_in, field_out, sendtypes_in, recvtypes_in)

    COMPLEX(num), DIMENSION(1-ng:), TARGET, INTENT(IN) :: field_in
    COMPLEX(num), DIMENSION(1-ng:), TARGET, INTENT(OUT) :: field_out
    INTEGER, DIMENSION(0:), TARGET, INTENT(INOUT) :: sendtypes_in, recvtypes_in

    sendtypes => sendtypes_in
    recvtypes => recvtypes_in
    field_in1dcmplx => field_in
    field_out1dcmplx => field_out
    CALL redblack_main(do_send1dcmplx, do_recv1dcmplx)

  END SUBROUTINE redblack1d_cmplx



  SUBROUTINE redblack2d_cmplx(field_in, field_out, sendtypes_in, recvtypes_in)

    COMPLEX(num), DIMENSION(1-ng:,1-ng:), TARGET, INTENT(IN) :: field_in
    COMPLEX(num), DIMENSION(1-ng:,1-ng:), TARGET, INTENT(OUT) :: field_out
    INTEGER, DIMENSION(0:), TARGET, INTENT(INOUT) :: sendtypes_in, recvtypes_in

    sendtypes => sendtypes_in
    recvtypes => recvtypes_in
    field_in2dcmplx => field_in
    field_out2dcmplx => field_out
    CALL redblack_main(do_send2dcmplx, do_recv2dcmplx)
  ...
  SUBROUTINE redblack3d_cmplx(field_in, field_out, sendtypes_in, recvtypes_in)
    ...
    COMPLEX(num), DIMENSION(1-ng:,1-ng:,1-ng:), TARGET, INTENT(IN) :: field_in
    COMPLEX(num), DIMENSION(1-ng:,1-ng:,1-ng:), TARGET, INTENT(OUT) :: field_out
    INTEGER, DIMENSION(0:), TARGET, INTENT(INOUT) :: sendtypes_in, recvtypes_in

    sendtypes => sendtypes_in
    recvtypes => recvtypes_in
    field_in3dcmplx => field_in
    field_out3dcmplx => field_out
    CALL redblack_main(do_send3dcmplx, do_recv3dcmplx)

  END SUBROUTINE redblack3d_cmplx

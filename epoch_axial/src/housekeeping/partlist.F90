...
CONTAINS
  ...
  SUBROUTINE pack_particle(array, a_particle)
    ...
    cpos = 1
    array(cpos:cpos+2) = a_particle%part_pos
    cpos = cpos+3
  ...
  SUBROUTINE unpack_particle(array, a_particle)
    ...
    cpos = 1
    a_particle%part_pos = array(cpos:cpos+2)
    cpos = cpos+3
  ...
  FUNCTION get_macro_part_volume(part)

    ! Calculate the volume occupied by a macro particle, factoring in the 
    ! macro-particle shape function, and reflection on r=0. In cylindrical
    ! co-ordinates, we calculate densities by dividing by the macro-particle 
    ! volume - not the cell volume

    TYPE(particle), POINTER, INTENT(IN) :: part 
    REAL(num) :: get_macro_part_volume
    REAL(num) :: part_r

    part_r = SQRT(part%part_pos(2)**2 + part%part_pos(3)**2)
    get_macro_part_volume = 2.0_num * pi * dx * dy * part_r
    
  END FUNCTION get_macro_part_volume

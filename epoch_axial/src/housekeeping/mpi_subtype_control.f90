...
CONTAINS
  ...
  !----------------------------------------------------------------------------
  ! create_current_field_mode_subtype - Creates the subtype corresponding to the
  ! current load balanced geometry, for complex 3D field-mode arrays
  !----------------------------------------------------------------------------

  FUNCTION create_current_field_mode_subtype(basetype)

    INTEGER :: create_current_field_mode_subtype
    INTEGER, INTENT(IN) :: basetype

    create_current_field_mode_subtype = &
        create_field_mode_subtype(basetype, nx, ny, nx_global_min, &
        ny_global_min)

  END FUNCTION create_current_field_mode_subtype



  !----------------------------------------------------------------------------
  ! create_current_field_mode_subarray - Creates the subarray corresponding to the
  ! current load balanced geometry, for complex 3D field-mode arrays
  !----------------------------------------------------------------------------

  FUNCTION create_current_field_mode_subarray(ng, basetype)

    INTEGER :: create_current_field_mode_subarray
    INTEGER, INTENT(IN) :: ng
    INTEGER, INTENT(IN) :: basetype

    create_current_field_mode_subarray = &
        create_field_mode_subarray(basetype, ng, nx, ny, n_mode)

  END FUNCTION create_current_field_mode_subarray
  ...
  !----------------------------------------------------------------------------
  ! create_field_mode_subtype - Creates a subtype representing the field modes 
  ! on the local processor for any arbitrary arrangement of an array covering 
  ! the entire spatial domain. Only used directly during load balancing
  !----------------------------------------------------------------------------

  FUNCTION create_field_mode_subtype(basetype, nx_local, ny_local, &
      cell_start_x_local, cell_start_y_local)
    ...
    INTEGER, INTENT(IN) :: cell_start_x_local
    INTEGER, INTENT(IN) :: cell_start_y_local
    INTEGER :: create_field_mode_subtype
    INTEGER, DIMENSION(3) :: n_local, n_global, start

    n_local = (/nx_local, ny_local, n_mode/)
    n_global = (/nx_global, ny_global, n_mode/)
    start = (/cell_start_x_local, cell_start_y_local, 1/)

    create_field_mode_subtype = &
        create_3d_array_subtype(basetype, n_local, n_global, start)

  END FUNCTION create_field_mode_subtype
  ...
  FUNCTION create_field_mode_subarray(basetype, ng, n1, n2, n3)

    INTEGER, INTENT(IN) :: basetype, ng, n1, n2, n3
    INTEGER, DIMENSION(3) :: n_local, n_global, start
    INTEGER :: i, ndim, create_field_mode_subarray
    ...
    ! Output cells from 1:nx and 1:ny
    DO i = 1, 2
      start(i) = 1 + ng
      n_global(i) = n_local(i) + 2 * ng
    END DO

    ! Output every field mode
    start(3) = 1
    n_global(3) = n_local(3)

    create_field_mode_subarray = &
        create_3d_array_subtype(basetype, n_local, n_global, start)

  END FUNCTION create_field_mode_subarray

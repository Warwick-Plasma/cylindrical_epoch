...
CONTAINS
  ...
  SUBROUTINE non_uniform_load_particles(density, species, density_min, &
    ...
    TYPE(particle_species), POINTER :: species
    REAL(num), ALLOCATABLE :: real_count(:,:)
    ...
    REAL(num) :: real_count_total, real_count_total_global, real_count_average
    REAL(num) :: macro_vol, cell_r, npart_per_cell_average
    REAL(num) :: part_r, part_th, r_low
    ...
    ALLOCATE(real_count(1-ng:nx+ng, 1-ng:ny+ng))
    num_valid_cells_local = 0
    real_count_total = 0.0_num
    ...
    DO iy = 1, ny
      ! Calculate mean macro-particle volume in the current cell
      cell_r = REAL(iy-1,num) * dy + y_grid_min_local
      macro_vol = 2.0_num * pi * dx * dy * cell_r

      ! Calculate number of real particles in each cell
      DO ix = 1, nx
        IF (density(ix,iy) >= density_min) THEN
          num_valid_cells_local = num_valid_cells_local + 1
          real_count(ix,iy) = density(ix,iy) * macro_vol
          real_count_total = real_count_total + real_count(ix,iy)
        END IF
      END DO ! ix
    END DO ! iy
    ...
    CALL MPI_ALLREDUCE(real_count_total, real_count_total_global, 1, mpireal, &
        MPI_SUM, comm, errcode)
    real_count_average = real_count_total_global &
        / REAL(num_valid_cells_global, num)

    IF (pre_loading) THEN
      DO iy = 1, ny
      DO ix = 1, nx
        npart_per_cell = NINT(real_count(ix,iy) / real_count_average &
            * npart_per_cell_average)
        npart_per_cell_array(ix,iy) = &
            npart_per_cell_array(ix,iy) + INT(npart_per_cell)
      END DO ! ix
      END DO ! iy
      DEALLOCATE(real_count)
    ...
    DO ix = 1, nx
      ...
      DO WHILE(ASSOCIATED(current) .AND. ipart < npart_per_cell)
        ...
        r_low = (y(iy) - 0.5_num * dy)
        part_r = SQRT(r_low**2 + random() * ((r_low + dy)**2 - r_low**2)) 
        part_th = 2.0_num * pi * random()
        current%part_pos(2) = part_r * COS(part_th) 
        current%part_pos(3) = part_r * SIN(part_th)
    ...
    species%weight = real_count_total_global / npart_this_species
  ...
  SUBROUTINE load_particles(species, load_list)
    ...
    IF (npart_left > 0) THEN
      ...
      DO i = 1, npart_left
        ...
        part_r = y(cell_y) + (random() - 0.5_num) * dy
        part_th = 2.0_num * pi * random()
        current%part_pos(2) = part_r * COS(part_th)
        current%part_pos(3) = part_r * SIN(part_th)
  ...
  SUBROUTINE setup_particle_density(density_in, species, density_min, &
    ...
    REAL(num) :: macro_part_volume, part_th
    TYPE(particle_list), POINTER :: partlist
...
#include "particle_to_grid.inc"
    ...
    DO WHILE(ipart < partlist%count)
      ...
      ! Calculate the volume occupied by the macro-particle
      macro_part_volume = get_macro_part_volume(current)
      current%weight = current%weight * macro_part_volume &
          / npart_in_cell(cell_x,cell_y)

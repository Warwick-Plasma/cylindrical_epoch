...
CONTAINS
  ...
  SUBROUTINE output_routines(step, force_write)   ! step = step index
    ...
    DO iprefix = 1,SIZE(file_prefixes)
      ...
      CALL write_mode_field(c_dump_exm, code, 'exm_real', &
          'Electric Field Modes/Exm/real', 'V/m', c_stagger_exm, REAL(exm,num))
      CALL write_mode_field(c_dump_erm, code, 'erm_real', &
          'Electric Field Modes/Erm/real', 'V/m', c_stagger_erm, REAL(erm,num))
      CALL write_mode_field(c_dump_etm, code, 'etm_real', &
          'Electric Field Modes/Etm/real', 'V/m', c_stagger_etm, REAL(etm,num))

      CALL write_mode_field(c_dump_exm, code, 'exm_imag', &
          'Electric Field Modes/Exm/imag', 'V/m', c_stagger_exm, AIMAG(exm))
      CALL write_mode_field(c_dump_erm, code, 'erm_imag', &
          'Electric Field Modes/Erm/imag', 'V/m', c_stagger_erm, AIMAG(erm))
      CALL write_mode_field(c_dump_etm, code, 'etm_imag', &
          'Electric Field Modes/Etm/imag', 'V/m', c_stagger_etm, AIMAG(etm))

      CALL write_mode_field(c_dump_bxm, code, 'bxm_real', &
          'Magnetic Field Modes/Bxm/real', 'T', c_stagger_bxm, REAL(bxm,num))
      CALL write_mode_field(c_dump_brm, code, 'brm_real', &
          'Magnetic Field Modes/Brm/real', 'T', c_stagger_brm, REAL(brm,num))
      CALL write_mode_field(c_dump_btm, code, 'btm_real', &
          'Magnetic Field Modes/Btm/real', 'T', c_stagger_btm, REAL(btm,num))

      CALL write_mode_field(c_dump_bxm, code, 'bxm_imag', &
          'Magnetic Field Modes/Bxm/imag', 'T', c_stagger_bxm, AIMAG(bxm))
      CALL write_mode_field(c_dump_brm, code, 'brm_imag', &
      ...
      CALL write_mode_field(c_dump_jrm, code, 'jrm_real', &
          'Current Modes/Jrm/real', 'A/m^2', c_stagger_jrm, REAL(jrm,num))
      CALL write_mode_field(c_dump_jtm, code, 'jtm_real', &
          'Current Modes/Jtm/real', 'A/m^2', c_stagger_jtm, REAL(jtm,num))

      CALL write_mode_field(c_dump_jxm, code, 'jxm_imag', &
          'Current Modes/Jxm/imag', 'A/m^2', c_stagger_jxm, AIMAG(jxm))
      CALL write_mode_field(c_dump_jrm, code, 'jrm_imag', &
  ...
  SUBROUTINE write_mode_field(id, code, block_id, name, units, stagger, array)
    ...
    IF (id == c_dump_jxm .OR. id == c_dump_jrm .OR. id == c_dump_jtm) THEN 
      current_dump = .TRUE. 
    ELSE 
      current_dump = .FALSE. 
    END IF    
    ...
    IF (convert) THEN
      ...
      IF (current_dump) THEN
        subarray = subarray_mode_big_r4
      ELSE

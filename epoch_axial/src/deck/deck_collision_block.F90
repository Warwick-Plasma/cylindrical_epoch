...
CONTAINS
  ...
  FUNCTION collision_block_handle_element(element, value) RESULT(errcode)
    ...
    IF (str_cmp(element, 'subsample_ci')) THEN
      subsample_ci = as_logical_print(value, element, errcode)
      RETURN
    END IF

    IF (str_cmp(element, 'subsample_ci_number')) THEN
      subsample_ci_number = as_real_print(value, element, errcode)
      RETURN
    END IF

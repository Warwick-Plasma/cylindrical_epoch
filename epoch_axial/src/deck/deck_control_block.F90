...
CONTAINS
  ...
  SUBROUTINE control_deck_initialise
    ...
    got_grid(:) = .FALSE.
    got_grid(3) = .TRUE. ! y_min = 0 in cylindrical co-ordinates
    y_min = 0.0_num
    got_time = .FALSE.
  ...
  FUNCTION control_block_handle_element(element, value) RESULT(errcode)
    ...
    ELSE IF (str_cmp(element, 'n_mode')) THEN
      n_mode = as_integer_print(value, element, errcode)

    ELSE IF (str_cmp(element, 'max_mode')) THEN
      n_mode = as_integer_print(value, element, errcode) + 1

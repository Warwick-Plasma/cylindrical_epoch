...
CONTAINS
  ...
  FUNCTION io_block_handle_element(element, value) RESULT(errcode)
    ...
    ELSE IF (str_cmp(element, 'exm')) THEN
      elementselected = c_dump_exm

    ELSE IF (str_cmp(element, 'erm')) THEN
      elementselected = c_dump_erm

    ELSE IF (str_cmp(element, 'etm')) THEN
      elementselected = c_dump_etm

    ELSE IF (str_cmp(element, 'bxm')) THEN
      elementselected = c_dump_bxm

    ELSE IF (str_cmp(element, 'brm')) THEN
      elementselected = c_dump_brm
    ...
    ELSE IF (str_cmp(element, 'jrm')) THEN
      elementselected = c_dump_jrm
...
#define PARTICLE_ID
      ...
      IF (IAND(mask, c_io_averaged) /= 0) THEN
        ...
        IF (mask_element == c_dump_jy) bad = .FALSE.
        IF (mask_element == c_dump_jz) bad = .FALSE.
        IF (mask_element == c_dump_exm) bad = .FALSE.
        IF (mask_element == c_dump_erm) bad = .FALSE.
        IF (mask_element == c_dump_etm) bad = .FALSE.
  ...
  SUBROUTINE set_restart_dumpmasks
    ...
    io_block%dumpmask(c_dump_erm) = &
        IOR(io_block%dumpmask(c_dump_erm), c_io_restartable)
    io_block%dumpmask(c_dump_etm) = &
        IOR(io_block%dumpmask(c_dump_etm), c_io_restartable)
    io_block%dumpmask(c_dump_bxm) = &
        IOR(io_block%dumpmask(c_dump_bxm), c_io_restartable)
    io_block%dumpmask(c_dump_brm) = &

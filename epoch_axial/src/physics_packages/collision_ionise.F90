...
MODULE collision_ionise
  ...
  ! Cylindrical geometry correction terms
  REAL(num) :: cell_volume
...
CONTAINS
    ...
    DO ispecies = 1, n_species
      ...
      DO jspecies = ispecies, n_species
        ...
        IF (run_coll_ionisation) THEN
            cell_r_centre = ABS(REAL(iy - 1, num) * dy + y_grid_min_local)
            cell_volume = 2.0_num * pi * dx * dy * cell_r_centre 
  ...
  SUBROUTINE collision_ionisation_ei(ion_species, el_species, list_e_in, &
    ...
    TYPE(particle), POINTER :: electron, ion, ejected_electron, sub_ion
    ...
    REAL(num) :: sub_weight, sub_ion_weight
    LOGICAL :: run_subsample
    INTEGER :: sub_count
    ...
    DO i_ion = 1, ion_count
      ...
      ion => ion%next
    END DO
    inv_cell_volume = 1.0_num / cell_volume
    ...
    DO i_el = 1, e_count
      ...
      first_pass = .TRUE.
      IF (subsample_ci) run_subsample = .TRUE.
      DO WHILE (e_collide_again)
        ...
        IF (sec_no > ion_weight) THEN
          ...
          ! No need to sub-sample if we already have ionisation
          IF (subsample_ci) run_subsample = .FALSE.
        ELSE
          ! Weight of macro-ion >= expected number of secondary e-
          IF (.NOT. run_subsample) THEN
            ! Sample the probability of ionisation for the macro-ion
            create_secondary = (random() <= sec_no / ion_weight)
          ELSE 
            ! Split macro-ion into sub-macro-ions, and see how many ionise 
            sub_weight = ion_weight / subsample_ci_number
            sub_count = FLOOR(sec_no / sub_weight)
            IF (random() < (sec_no - sub_count*sub_weight) / sub_weight) &
              sub_count = sub_count + 1
            create_secondary = sub_count > 0
          END IF
        ...
        IF (create_secondary .AND. .NOT. run_subsample) THEN
        ...
        ! Partially ionise macro-ion
        IF (create_secondary .AND. run_subsample) THEN
          sub_ion_weight = sub_weight * REAL(sub_count, num)
          CALL generate_secondary_electron(sec_ke_i, el_dir, ejected_electron, &
              ion, ion_at_rest)
#ifndef PER_SPECIES_WEIGHT
          ion%weight = ion%weight - sub_ion_weight
          ejected_electron%weight = sub_ion_weight
#endif
          ...
          ! Create a new macro-ion and add to the ionised list
          CALL generate_sub_ion(ion, sub_ion, sub_ion_weight)
          CALL add_particle_to_partlist(list_i_ionised, sub_ion)
          NULLIFY(sub_ion)

          ! Reduce background ion number density
          n_i = n_i - sub_ion_weight * inv_cell_volume
        END IF
  ...
  SUBROUTINE generate_sub_ion(ion, sub_ion, sub_ion_weight)

    ! When subsampling, we treat the original macro-ion as several sub-ions.
    ! We do not destroy the original macro-ion, but we do reduce its weight and 
    ! create a new ionised macro-ion with the remaining weight. This subroutine
    ! generates the new macro-ion

    REAL(num) :: sub_ion_weight
    TYPE(particle), POINTER, INTENT(OUT) :: sub_ion
    TYPE(particle), POINTER, INTENT(IN) :: ion

    CALL create_particle(sub_ion)

    ! Sub ion has the same properties as the main ion, except the weight
    sub_ion%part_p = ion%part_p
...
#endif
#ifdef PER_PARTICLE_CHARGE_MASS
    sub_ion%charge = ion%charge + q0
    sub_ion%mass = ion%mass - m0
#endif
#ifdef PARTICLE_DEBUG
    sub_ion%processor = rank
    sub_ion%processor_at_t0 = rank
#endif

  END SUBROUTINE generate_sub_ion

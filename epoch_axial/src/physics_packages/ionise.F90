...
CONTAINS
  ...
  SUBROUTINE multiphoton_tunnelling_bsi
    ...
    REAL(num) :: rate, e_part_mag, time_left, sample
    INTEGER :: im
    REAL(num) :: ex_part, ey_part, ez_part, er_part, et_part
    REAL(num) :: part_r
    COMPLEX(num) :: exp_min_itheta, exp_min_imtheta, exp_itheta, exp_imtheta 
    REAL(num) :: part_x_local, part_r_local
    REAL(num) :: dfac, dfac1, weight, j_ion(3)
    REAL(num) :: jx_ion, jy_ion, jz_ion, jr_ion, jt_ion
    REAL(num) :: gx(sf_min:sf_max), hx(sf_min:sf_max)
    REAL(num) :: gy(sf_min:sf_max), hy(sf_min:sf_max)
    REAL(num) :: cell_x_r, cell_frac_x, idx
    REAL(num) :: cell_y_r, cell_frac_y, idy
    ...
    dfac1 = fac**2 / (dt * 2.0_num * pi * dx * dy)
    ...
    DO i = 1, n_species
      ...
      DO WHILE(ASSOCIATED(current))
        ...
        part_x_local  = current%part_pos(1) - x_grid_min_local
        part_r = SQRT(current%part_pos(2)**2 + current%part_pos(3)**2)
        part_r_local  = part_r - y_grid_min_local
        exp_min_itheta = &
            (current%part_pos(2) - imagi * current%part_pos(3)) / part_r 
        exp_itheta = 1.0_num / exp_min_itheta
...
#include "triangle/e_part.inc"
        ...
        IF (current_state /= i) THEN
          ...
          IF (ABS(j_ion(1)) > c_tiny &
            ...
            jr_ion =  jy_ion *  REAL(exp_itheta) + jz_ion * AIMAG(exp_itheta)
            jt_ion = -jy_ion * AIMAG(exp_itheta) + jz_ion *  REAL(exp_itheta) 

            exp_imtheta = 1.0_num
            DO im = 0, n_mode-1
              DO iy = sf_min, sf_max
              DO ix = sf_min, sf_max
                jxm(cell_x1+ix,cell_y2+iy,im) = jxm(cell_x1+ix,cell_y2+iy,im) &
                    + gx(ix) * hy(iy) * jx_ion * exp_imtheta
                jrm(cell_x2+ix,cell_y1+iy,im) = jrm(cell_x2+ix,cell_y1+iy,im) &
                    + hx(ix) * gy(iy) * jr_ion * exp_imtheta
                jtm(cell_x2+ix,cell_y2+iy,im) = jtm(cell_x2+ix,cell_y2+iy,im) &
                    + hx(ix) * hy(iy) * jt_ion * exp_imtheta
              END DO
              END DO
              exp_imtheta = exp_imtheta * exp_itheta
              IF (im==0) exp_imtheta = exp_imtheta * 2.0_num
            END DO
          END IF
        END IF
        current => next

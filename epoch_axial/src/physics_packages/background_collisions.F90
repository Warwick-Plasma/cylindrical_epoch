...
CONTAINS
  ...
  SUBROUTINE run_background_collisions
    ...
    REAL(num) :: part_x_local, part_r, part_r_local
    ...
    DO i_fast = 1, n_species
      ...
      DO WHILE(ASSOCIATED(fast))
        ...
        part_x_local = fast%part_pos(1) - x_grid_min_local 
        part_r = SQRT(fast%part_pos(2)**2 + fast%part_pos(3)**2)
        part_r_local = part_r - y_grid_min_local
#ifdef PARTICLE_SHAPE_TOPHAT
        cell_x = FLOOR(part_x_local / dx) + 1
        cell_y = FLOOR(part_r_local / dy) + 1
#else
        cell_x = FLOOR(part_x_local / dx + 1.5_num)
        cell_y = FLOOR(part_r_local / dy + 1.5_num)
#endif
